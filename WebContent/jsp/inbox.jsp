...
<input type="hidden" name="txthid" value="inbox"/>
<%if(!rs.next())
{
%>
<div class="alert alert-info" align="center" style="color:red">There are no messages!!</div>
<%} 
else
{ %>
<table class="table" style="width: 100%">
<tr><th>Delete</th><th>Sender ID</th><th>Subject</th><th> Date</th></tr>
<%while(rs.next())
{
	String msid=rs.getString("msgid");

%>
<tr><th><input type="checkbox" name="chkdel" value="<%=rs.getString("msgid")%>"/></th>
<th><%=rs.getString("senderid") %></th>
<th><a href="/Etalent/jsp/showmessage.jsp?mid=<%=msid%>&pname=inbox"><%=rs.getString("subject")%>
</a></th>
<th><%=rs.getString("date")%></th></tr>
<%} %>
<tr><th colspan="4" align="center"><button type="submit" class="btn btn-info btn-lg">Delete</button>
</th></tr></table>
</form>
</div>
<h1><a href="/Etalent/jsp/candmessages.jsp">No,Thanks</a></h1>
<%}} %>
</body>
</html>